# -*- coding: utf-8 -*-
"""
Build Group C snapshot: merge latest tags + factors_latest, compute group_c
representatives and deviations. Returns C snapshot DataFrame; does not save history.
Optional: save C latest-only file. Independent runnable.
"""
...
def build_group_c_snapshot_df(
    logic_dir: str | Path | None = None,
    data_dir: str | Path | None = None,
) -> pd.DataFrame:
    """
    Build Group C snapshot DataFrame: latest tags + factors merge, group_c reps and deviations.
    ...
    log_snapshot_as_of_date_sanity(snapshot_df, label="group_c_snapshot")
...
def main() -> None:
    ...
    snapshot_df = build_group_c_snapshot_df(logic_dir=DEFAULT_LOGIC_DIR, data_dir=DEFAULT_DATA_DIR)
